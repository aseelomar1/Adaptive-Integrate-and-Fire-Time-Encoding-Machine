...
w = 2;  % Window size for bias adaptation
biasbits =4; %bias allocated for the bias vetor for AIF-TEM
bits =10; % bits usef for quantiization stage.
...
%% Quantization
tmaxif = ((kappa * th_c) / (bias_if - cmax));
tminif = ((kappa * th_c) / (bias_if + cmax));
tn_if_quant = quantizationTEM(tn_if, tmaxif, tminif, bits, min(t));

%% IF-TEM Signal Recovery
xr_if = recover_TEM(t', tn_if_quant, 2 * pi * f, bias_if, kappa, th_c);
...
totalNBitsif = numel(trIndices_if)*bits;
if verbose
   disp(['The quantization MSE in dB for IF-TEM: ', num2str(mse_db_if), ' with total number of bits: ', num2str(totalNBitsif)]);
end
...
OS_a =  numel(trIndices_aif)/((max(t)-min(t))/(1/(2*f)));% calcuate Oversampling of AIF-TEM
 ...
 %% Quantization
tmaxaif = ((kappa * th_c) / (bias_aif - cmax));
tminaif = ((kappa * th_c) / (bias_aif + cmax));
tn_aif_quant = quantizationTEM(tn_aif, tmaxaif, tminaif, bits, min(t));

%% AIF-TEM Signal Recovery
xr_aif = recover_TEM(t', tn_aif_quant, 2 * pi * f, bias_adaptive, kappa, th_c);
...
totalNBitsaif =numel(trIndices_aif)*(bits+biasbits);
if verbose
     disp(['The quantization MSE in dB for AIF-TEM: ', num2str(mse_db_aif), ' with total number of bits: ', num2str(totalNBitsaif)]);
